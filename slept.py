...
def insert_times(sleep_times, sleep_date):
    for p in sleep_times:
        db.execute("INSERT INTO sleep_times VALUES ('" +
                sleep_date.strftime("%Y-%m-%d") + "','" + p[0].strftime("%H:%M") +
                "','" + p[1].strftime("%H:%M") + "')")
...
def get_date_times(date):
    sleep_list = []
    for row in db.execute("SELECT start_time, end_time FROM sleep_times WHERE date='"
            + date.strftime("%Y-%m-%d") + "'"):
...
def delete_date_times(date):
    for row in db.execute("SELECT date, start_time, end_time FROM sleep_times WHERE date='"
            + date.strftime("%Y-%m-%d") + "'"):
        print('Deleted sleep time ' + row[1] + ' to ' + row[2] + ' on ' + row[0])
...
def file_db_setup():
    'creates config file, database, and table'
    # not currently used
    #try:
...
def main():
    ...
    if (date_in_db(sleep_date)):
        if (args.replace):
            ...
            print('"', sleep_date.strftime('%Y-%m-%d'), '" already in database. ',
                    'Use \'-R\' to replace, or \'-a\' to add new times.',
                    sep="", file=sys.stderr)
            sys.exit()

    # convert input to a list of sleeptime tuples
    sleep_times = sorted(list(map(timespec_conv, args.timespecs)), key=sleep_time_key)
